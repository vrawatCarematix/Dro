    ...
    static func getAllSurveySchedules( ) -> [SurveyScheduleDatabaseModel]{
        ...
        if sqlite3_prepare_v2(db, GET_ALL_SURVEY_SCHEDLUE , -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW{
                ...
                surveySchedules.surveyDate = utcToLocalEpoch(utcDateEpoch:Int(sqlite3_column_int64(statement, 0)))
                surveySchedules.sessionCount = Int(sqlite3_column_int64(statement, 1))
                surveySchedules.startTime = utcToLocalEpoch(utcDateEpoch:Int(sqlite3_column_int64(statement, 2 )  ))
                surveySchedules.endTime =  utcToLocalEpoch(utcDateEpoch: Int(sqlite3_column_int64(statement, 3) ))
                ...
                surveySchedules.actualEndTime = utcToLocalEpoch(utcDateEpoch: Int(sqlite3_column_int64(statement, 15) ))
    ...
    static func getAllViewDro( ) -> [DroInfoListModel]{
        ...
        if sqlite3_prepare_v2(db, GET_ALL_VIEW_DRO , -1, &statement, nil) == SQLITE_OK {
            while sqlite3_step(statement) == SQLITE_ROW{
                ...
                droInfoList.endTime = utcToLocalEpoch(utcDateEpoch:Int(sqlite3_column_int64(statement, 6)  ))
