...
//  DRO
//
//  Created by Carematix on 04/05/18.
...
class AudioQuestionCell: UITableViewCell {
    ...
    @IBOutlet weak var timeSeekbar: CustomSlider!
    ...
    @objc func updateTime(_ timer: Timer) {
        if let time = (self.avPlayer?.currentItem?.currentTime().seconds) {
           timeSeekbar.value = Float(time)
        }
